  ...
  */
  if ($_SERVER['REQUEST_METHOD'] == 'GET'){
    if (!isset($_GET['paciente']) && !isset($_GET['sesion'])){
      ...
      $sql = $dbConn->prepare("SELECT m.tms,m.tmf 
      FROM medidas_aerodinamicas m, sesiones s 
      WHERE s.id_sesion=m.id_sesion
      ORDER BY id_aerodinamica DESC LIMIT 1");
      $sql->execute();
      ...
      $sql = $dbConn->prepare("SELECT m.tms,m.tmf 
      FROM medidas_aerodinamicas m, sesiones s 
      WHERE s.id_sesion=m.id_sesion AND s.id_paciente=:paciente AND s.id_sesion=:sesion
      ORDER BY id_aerodinamica DESC LIMIT 1");
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $sql = $dbConn->prepare("INSERT INTO medidas_aerodinamicas (id_sesion,tms,tmf) VALUES (:sesion,:tms,:tmf)");
